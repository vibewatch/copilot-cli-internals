...
The current artifact is `1.0.82`; source anchors and behavioral pages below describe that baseline directly.
...
|---|---|---|---|
| Context engineering | What should be model-visible for this turn, and how should it be shaped? | System prompt builders, instruction discovery, skills and custom-agent prompts, attachments, MCP prompts/resources, git/IDE/session metadata, memory/context board, compaction, request-time prompt trimming, provider request mapping. | [`prompt-sources.md`](../02-context-model-loop/prompt-sources.md), [Prompt catalog](../02-context-model-loop/prompt-catalog.md), [`custom-agents-and-skills-packaging.md`](../06-agents-automation/custom-agents-and-skills-packaging.md), [`memory-and-context-board.md`](../02-context-model-loop/memory-and-context-board.md), [`conversation-compaction.md`](../02-context-model-loop/conversation-compaction.md) |
...
This distinction is useful when reading the rest of the wiki: if a question is about **what the model sees**, start in [Context and model loop](../02-context-model-loop/README.md); if it is about **how the model is allowed to act**, start in [Runtime lifecycle](../01-runtime-lifecycle/README.md), [Tools, integrations, and security](../03-tools-integrations-security/README.md), or [Agents and automation](../06-agents-automation/README.md).
...
| Indexed repository search | Eligible grep/search startup | Repository-keyed tgrep server/index beneath existing grep tools, with readiness polling and ripgrep fallback. | `TGREP`, `USE_TGREP`, git root, filesystem type, file threshold, server health. |
...
| MCP | `copilot mcp`, workspace/plugin config, live session APIs | Stdio/HTTP/SSE servers, resources, GitHub MCP selections, deferred tools, live reload/start/stop, elicitation, sampling. | MCP policy, enterprise allowlists, MCP feature gates. |
| Plugins and extensions | `copilot plugin`, `/plugins`, plugin dirs, extension host | Marketplaces, SHA-pinned sources, skills, agents, hooks, MCP/LSP servers, and plugin-packaged SDK extensions. | `EXTENSIONS`, folder trust, plugin enablement. |
| Models and providers | `--model`, model picker, BYOK env vars | Dynamic Copilot catalog, reasoning effort, auto mode, current Claude/GPT/Gemini/Kimi families, HTTP/WebSocket provider routing. | Auth/provider config, plan tier, model availability. |
