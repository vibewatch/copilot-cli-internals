...
| Main system prompt | `buildSystemPrompt(...)`, general-purpose wrapper | `aX(...)`, `xvt(...)`, `ime(...)` | `app.js` 479-505, 2755-2759 | Builds the default system prompt for top-level and general-purpose agent sessions. |
...
| Task tool instructions | Task tool factory and native input preparation | `S.toolTaskPrepareInput(...)`, `agent_type` schema/dispatch | `app.js` 374-477 | Defines subagent usage rules, background-agent guidance, and the model-visible task tool schema. |
| Built-in agents | Built-in catalog arrays and YAML definitions | `$_t`, `ozn`, `qLe` | `app.js` 324 plus `copilot-cli-pkg/definitions/*.agent.yaml` | Built-in agent descriptions are listed in `app.js`; full prompts are loaded from YAML definition files. |
...
| Environment context | `buildAgentEnvironmentContext(...)`, `buildCliEnvironmentContext(...)` | `h7n(...)`, `S.promptsCliEnvironmentContext(...)` | `app.js` 479-505 | Adds cwd, git root, OS, directory snapshot, available tools, repository, and connected IDE context. |
