...
- `--cloud` requests a cloud sandbox-backed session. This is separate from local command sandboxing via `/sandbox`; see [`sandboxing.md`](../03-tools-integrations-security/sandboxing.md).
...
Conversation compaction is part of this session-history layer: successful compaction emits session events, mutates reconstructed chat history during replay, and can persist checkpoint summaries. See [`conversation-compaction.md`](../02-context-model-loop/conversation-compaction.md) for the implementation details.
...
Cloud sessions create or attach to a sandbox-backed environment rather than only using the local workspace. This section is about **remote cloud sandbox sessions**. The local `/sandbox` command toggles shell-process sandboxing inside local sessions and is documented in [`sandboxing.md`](../03-tools-integrations-security/sandboxing.md).
