...
|---|---|---|---|
| Startup and runtime mode routing | `copilot` argv, stdin, TTY, global options | `RootProgram`, `y1`, `NIn`, `oCe`, `u9r`, `--server`, `--headless`, `--acp` | [`main-feature-map.md`](../00-start-here/main-feature-map.md), [`mode-dispatch-and-runtime-startup.md`](../01-runtime-lifecycle/mode-dispatch-and-runtime-startup.md) |
...
| Prompt and context assembly | session initialization, subagent creation, slash command prompt macro, provider request | `aX`, `xvt`, `ime`, `m7n`, `h7n`, `S.prompts*`, custom-instruction and skill loaders | [`prompt-sources.md`](../02-context-model-loop/prompt-sources.md), [`prompt-catalog.md`](../02-context-model-loop/prompt-catalog.md) |
...
| Runtime tool assembly | session/subagent tool initialization or tool surface invalidation | `buildSettingsAndTools`, `initializeAndValidateTools`, native tool descriptors, `session.tools_updated` | [`runtime-tool-assembly-and-filtering.md`](../03-tools-integrations-security/runtime-tool-assembly-and-filtering.md) |
...
| Subagent and task orchestration | main model calls `task`; slash macro steers toward an agent | `nR`, `S.toolTaskPrepareInput`, `SessionAgentExecutor.execute() called`, `subagent.completed` | [`agent-task-orchestration.md`](../06-agents-automation/agent-task-orchestration.md), [`built-in-agents.md`](../06-agents-automation/built-in-agents.md) |
| Integrations and extension loading | MCP config, plugins, SDK extension gate, IDE bridge | `McpHost`, plugin loaders, `setupExtensionsForSession`, `session.extensions_loaded`, `callIdeTool` | [`mcp-host-transport-and-tools.md`](../03-tools-integrations-security/mcp-host-transport-and-tools.md), [`plugins-extensions-and-capabilities.md`](../03-tools-integrations-security/plugins-extensions-and-capabilities.md), [`copilot-sdk-extension-bridge.md`](../03-tools-integrations-security/copilot-sdk-extension-bridge.md), [`ide-lsp-editor-integration.md`](../03-tools-integrations-security/ide-lsp-editor-integration.md) |
| Permissions, policy, and sandbox boundaries | tool/path/URL/MCP/hook/shell action needs approval or policy check | `PermissionService`, `permissionRequest`, `allow-tool`, `deny-tool`, content exclusion strings, sandbox strings | [`tool-path-url-permissions.md`](../03-tools-integrations-security/tool-path-url-permissions.md), [`content-exclusion-and-redaction.md`](../03-tools-integrations-security/content-exclusion-and-redaction.md), [`hooks-events-and-automation.md`](../03-tools-integrations-security/hooks-events-and-automation.md), [`sandboxing.md`](../03-tools-integrations-security/sandboxing.md) |
| Operations, diagnostics, and shutdown | diagnostic command, telemetry/logging event, update check, signal/shutdown | `/diagnose`, `/collect-debug-logs`, `ShutdownService`, `OpenTelemetry`, update strings | [`diagnostics-feedback-debug-bundles.md`](../05-hosted-agent-ops/diagnostics-feedback-debug-bundles.md), [`telemetry-update-and-shutdown.md`](../05-hosted-agent-ops/telemetry-update-and-shutdown.md) |
...
5. **Map exact current anchors to semantic aliases.** For example, `nR=class{tasks=new Map` identifies `TaskRegistry`; `S.toolTaskPrepareInput(...)` identifies task-tool preparation; the `$_t`/`ozn`/`qLe` arrays identify the built-in catalog.
...
- Generated “first line” values are raw first hits. Earlier atlas baselines used short minified seeds such as `B3`, `I6n`, `nHn`, `HCr`, and `dZ`; those collided with unrelated bundled code. Current known-anchor and main-path seeds use exact declarations, preserved method names, or unique runtime strings. Ad hoc searches for short symbols still require nearby source confirmation.
...
- JSON-RPC-ish methods are identified by prefix/string shape; confirm protocol behavior in [`embedded-server-acp-protocol.md`](../01-runtime-lifecycle/embedded-server-acp-protocol.md), [`api-and-session-event-schemas.md`](../04-sessions-persistence-remote/api-and-session-event-schemas.md), or SDK docs before treating them as public methods.
