...
Secret scanning overlaps with [content exclusion and redaction](../05-security-and-policy/content-exclusion-and-redaction.md), but its role here is different: it is a completion/commit-time safety check for newly introduced file contents rather than a general transcript redaction mechanism.
