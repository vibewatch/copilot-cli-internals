...
It intentionally does not replace the focused pages on [session support](./session-support-implementation.md), [SessionFs](./session-fs-provider-and-state-files.md), [session-store indexing](./session-store-sqlite-indexing.md), [UI projection](./system-events-and-ui-projection.md), or [runtime tool assembly](../04-tools-and-integrations/runtime-tool-assembly-and-filtering.md). Instead, it is the missing reader path for following one conversation from startup to durable state and visible UI.
...
Tool state is therefore session state, but its detailed mechanics live in [Runtime tool assembly and filtering](../04-tools-and-integrations/runtime-tool-assembly-and-filtering.md). The session page should treat it as a lifecycle phase rather than a separate product.
...
- [Runtime tool assembly and filtering](../04-tools-and-integrations/runtime-tool-assembly-and-filtering.md) explains the tool configuration phase in detail.
- [Observability, update, and shutdown workflows](../08-operations-and-research/observability-update-shutdown.md) covers logs, telemetry, and graceful exit.
