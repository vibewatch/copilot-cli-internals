...
| Fleet mode prompt | `app.js` around line 4363 | High | `FLEET_MODE_PROMPT` is a static template that can optionally append the user's request. See [`fleet-mode.md`](../07-agents-and-automation/fleet-mode.md). |
...
Prompt wording that mentions a sandbox is not the same as local command sandbox enforcement. Local enforcement is wired through `settings.sandbox.enabled`, shell configuration, and the MXC spawn path; see [`sandboxing.md`](../05-security-and-policy/sandboxing.md).
...
The provider adapter layer is extractable as structure, but it does not change the fact that the content was composed from runtime-specific sources before the adapter saw it. See [`model-api-routing.md`](../06-models-and-reliability/model-api-routing.md) for the dedicated endpoint-selection and wire-format analysis.
...
Related docs: [Main feature map](../00-overview/main-feature-map.md), [`memory-and-context-board.md`](./memory-and-context-board.md), [`tui-and-slash-commands.md`](../01-runtime-and-ui/tui-and-slash-commands.md), [`sandboxing.md`](../05-security-and-policy/sandboxing.md), [`agent-task-orchestration.md`](../07-agents-and-automation/agent-task-orchestration.md), [`fleet-mode.md`](../07-agents-and-automation/fleet-mode.md), and [`models-providers-auth.md`](../06-models-and-reliability/models-providers-auth.md).
